...
function deObjetoAarray(objeto) {
   ...
   // Tu código:
   var Akeys = Object.keys(objeto);
   var resultado = [];

   Akeys.forEach(element => {
...
function capToFront(string) {
   ...
   // Tu código:
   var pSeparada = string.split('');
   var mayus = [];
   var minus = [];
   
   for(let i = 0; i < pSeparada.length; i++) {
      if(pSeparada[i] === pSeparada[i].toUpperCase()){
         mayus.push(pSeparada[i]);
      } else {
         minus.push(pSeparada[i]);
      }
   }
